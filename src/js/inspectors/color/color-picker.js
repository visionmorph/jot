/* Color picker popup, rendering, positioning, and pointer gestures. */
...
colorPickerOpacityInput?.addEventListener("keydown", (event) => {
  if (["ArrowUp", "ArrowDown"].includes(event.key) && colorPickerOpacityInput instanceof HTMLInputElement) {
    event.preventDefault();
    const direction = (event.key === "ArrowUp" ? 1 : -1) * (event.shiftKey ? 10 : 1);
    colorPickerOpacityInput.value = String(normalizeColorOpacity(Number(colorPickerOpacityInput.value) + direction));
    colorPickerOpacityInput.dispatchEvent(new Event("input", { bubbles: true }));
    return;
  }
  if (event.key !== "Enter") return;
  colorPickerFieldEditing = false;
